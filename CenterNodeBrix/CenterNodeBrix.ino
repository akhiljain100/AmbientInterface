...
// When "csma" command starts, the data will be retrieved until "stop" cmd is active.   //
...
#define NWK_JOIN_FRAME    0x02    // Sent to satellite nodes by the center node for joining the communication
#define NWK_JOIN_CONFIRM  0x03    // Received from satellite node (Confirmation frame)
#define STOP_CMD_FRAME    0x04    // To stop the communication in between center and slave, once this is sent the entire communication is stopped. To begin again, select the patch you want to start with 
// Frames for selecting the mode. Mode can be discrete or continuous for different satellite/slave nodes based on the 
// patch you select i.e, "a" patch will assign both continuous and discrete mode alternatievly to satellite brix. 
...
char patch;
// Parameter to send to brix, we can add more parameters but we have to increase the parameters in patch, See patch 'a' implementation 
...
void setup(){
  ...
  myBrick.initialize();
  mySensor.initialize();
  // At the beginning we set command and data available to false, the moment we receive any data from slave node we set data available to true and if we type some command in serial, then we set 
  // the command available to true.Once they are true, it checks the condition, for eg. if dataAVAILABLE is true, it checks the frame sent by slave and accordingly function 
  // Likewise for command available.
  ...
  // Broadcast the data with reset frame, if slave receive this packet it will send NWK_JOIN_FRAME for estabilishing the communication
...
void loop(){
  ...
  while(!dataAVAILABLE && !commandAVAILABLE){
    ...
    if(Serial1.available()>0){
      ...
      //Check if address is -ve (Sometimes I got bizzare data from brix) then parse the integer again
      ...
      if((receivedData[CTRL] == BACK_CENTER_FRAME || receivedData[CTRL] == NWK_JOIN_FRAME ) && receivedData[LEN] <= MAX_DATA_SIZE){
        /* Proceed if the frame is correct. i.e, make data available true and call different function based on the frame */
        dataAVAILABLE = true;
    ...
    // check for command input. Read the parameter values from command line. Right now just sending parameters for 3 patches. 
    // To stop the communication or getting data back from slave we dont use any parameter, but its implemented in the command line if-else condition
    if (Serial.available() > 0) {
      ...
      //if no further char is availible leave idle loop
  ...
  if(commandAVAILABLE){
    ...
    if (patch =='s'){
      ...
      delay(100);
        myBrick.sendInt(addressTable[i], transmittedData, 3); // broadcast
        delay(10);
      }
    }
    else if ( patch == 'a')
  ...
  if(dataAVAILABLE){ 
    ...
    // New satellite nodes inclusion to the Address Table of the Center Node and sending back the confirmation
    if(receivedData[CTRL] == NWK_JOIN_FRAME && receivedData[LEN] == 0){ // Check for network joining frame 
      myBrick.setRGB(0,0,255);
      ...
      Serial.println("");
      // send confirm frame with ID no., add the slave address to address table of Center node (Checking if the address already exist or not )
  ...
  // Data received from the brix
    else if(receivedData[CTRL] == BACK_CENTER_FRAME){ // Check for network joining frame 
